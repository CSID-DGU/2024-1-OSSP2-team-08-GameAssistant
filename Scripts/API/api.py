...
import time
from PyQt5.QtCore import QThread, pyqtSignal, QRunnable, QThreadPool, pyqtSignal, QObject

init_data_path='Data/TestJson/API/api_init_data.json'

class APIFactory:
    user_data_received = pyqtSignal(dict)
    match_data_received = pyqtSignal(dict)
...
class APICallRunnable(QRunnable):
    def __init__(self, api_factory, method, *args):
        super().__init__()
    ...
    def run(self):
        try:
            result = getattr(self.api_factory, self.method)(*self.args)
            if result:
                self.signals.result.emit(result)
            else:
                self.signals.error.emit("No Response")
        except Exception as e:
